...
- **屏幕截图翻译（OCR）**——Alt+Q（可改可禁用）或托盘菜单触发，屏幕压暗后框选区域，多模态大模型识别图内文字并流式翻译 + 提术语（智谱 GLM-4V-Flash 免费，复用翻译的 API Key）；PDF 扫描件、图片、视频内文字都能翻；多屏与高 DPI 缩放适配；识别与翻译两阶段执行（视觉模型纯识别 → 文本链翻译），翻译阶段同样吃缓存与备用服务，历史记录保存真实原文
- **网页版引擎（免费额度）**——划词/截图可切换到内嵌网页版 AI（默认 DeepSeek，登录一次长期有效）：自动注入指令、直接在弹出的站点页面看流式回复，回复完成自动归档翻译历史、【术语】段自动进生词本；Alt+Q 截图直接贴图识别、托盘开新会话/上传文档作上下文/切换引擎；无需 API Key；需完整版构建（见开发）
- **灵动界面**——弹窗淡入上浮、流式译文窗口平滑生长、术语表在译文就位后渐次展开；等待首字时显示呼吸骨架屏而非干等；错误弹窗轻微抖动提醒；弹窗为真圆角 + 柔和投影的浮层质感；截图框选带 L 形角标与实时像素尺寸；原文预览超长时可一键「全文」展开滚动；术语/状态字号跟随正文设置；操作按钮全面图标化（内嵌线性图标，hover 着色 / 按压回弹），设置页圆角卡片分组 + 自绘开关 + 侧栏滑动指示条
- **学习模式 + 术语解释**——译文之外额外输出专业术语表（读文献场景友好），术语每行带 ☆ 可一键收藏进生词本；Alt+E 划词提问术语解释（回答首行一句话定义，主词与关联术语自动进生词本，取词失败自动降级框选截图）；也支持仅译文的简洁模式；设置页可自定义六套提示词模板（划词/截图 × 学习/简洁、术语解释），`{text}` 占位符，留空回退内置默认
...
**网页版引擎（免费额度）**：设置 → 翻译服务 → 勾选「网页版引擎」，首次使用在弹出的窗口登录 DeepSeek（一次即可）。之后划词与 Alt+Q 截图自动走网页免费额度，直接在弹出的站点页面看流式回复，回复完成后自动归档进翻译历史（【术语】段自动进生词本）；托盘 → 网页翻译 可开新会话（清上下文）、上传文档（PDF/DOCX/TXT 等，作为后续翻译与问答的上下文）、切换引擎或手动对话。注意：该模式操作网页版界面，依赖站点页面结构（改版后需更新适配），仅供个人使用。
